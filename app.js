(function () {
  ...
  function quranCard() {
    ...
    return `<section class="faith-section quran-section" aria-labelledby="quran-title"><div class="section-title"><div><h2 id="quran-title">Quran</h2><p>${amount.toFixed(2)} of ${target} ${h(state.settings.quranUnit)}</p></div><strong class="progress-number">${percentage}%</strong></div><div class="progress-track" role="progressbar" aria-valuenow="${percentage}" aria-valuemin="0" aria-valuemax="100" aria-label="Quran progress"><span style="width:${percentage}%"></span></div><div class="quran-actions"><button class="secondary-button" data-quran-delta="-0.25" type="button" aria-label="Decrease by a quarter hizb" ${amount <= 0 ? "disabled" : ""}>− ¼</button><button class="primary-button" data-quran-delta="0.25" type="button" aria-label="Add a quarter hizb">+ ¼ hizb</button><button class="secondary-button" data-quran-set="${target}" type="button" ${amount >= target ? "disabled" : ""}>Complete target</button></div></section>`;
  }
